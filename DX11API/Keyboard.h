	...
	public:
		...
		struct State
		{
			...
			bool __cdecl IsKeyDown(Keys key) const
			{
				...
				if (key >= 0 && key <= 0xfe)
				{
					//将this强制转换为const uint32_T类型指针
					auto ptr = reinterpret_cast<const uint32_t*>(this);
					//根据 key 的 5 个最小有效位把 bf 变量设置为 2 的幂。
					unsigned int bf = 1u << (key & 0x1f);
					//若为有效按键，返回true，否则返回false
					return (ptr[(key >> 5)] & bf) != 0;
				}
				return false;
			}
			//监测按键是否没被按下，没被按下为true
